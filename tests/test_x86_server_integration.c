...
static void test_echo(int fd) {
  uint8_t frame[64];
  size_t flen = make_masked_frame(frame, sizeof(frame), (const uint8_t *)"Hi",
                                  2, WS_TEXT_FRAME);
...
static void test_ping_pong(int fd) {
  uint8_t frame[32];
  size_t flen = make_masked_frame(frame, sizeof(frame), NULL, 0, WS_PING_FRAME);
  send_all(fd, frame, flen);
...
static void test_close(int fd) {
  uint8_t frame[32];
  size_t flen =
      make_masked_frame(frame, sizeof(frame), NULL, 0, WS_CLOSING_FRAME);
  send_all(fd, frame, flen);
