...
static void test_unfragmented_text(void) {
  ...
  struct wsStreamCallbacks cbs = {
      .on_begin = on_begin, .on_data = on_data, .on_end = on_end, .user = &r};
...
static void test_fragmented_text(void) {
  ...
  assert(strcmp(r.log, "BEGIN(1,3);DATA(Hel);DATA(lo);END;") == 0);
}
...
static void test_ping(void) {
  uint8_t frame[64];
  size_t f = make_frame(frame, 1, WS_PING_FRAME, (const uint8_t *)"X", 1);
...
static void test_fragmented_binary(void) {
  ...
  assert(strcmp(r.log,
                "BEGIN(2,2);DATA(\x01\x02);DATA(\x03);DATA(\x04\x05);END;") ==
         0);
}
