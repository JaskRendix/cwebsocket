...
static size_t make_masked_frame(uint8_t opcode,
...
static void test_16bit_length(void)
{
    ...
    uint8_t frame[512];
    size_t frame_len = make_masked_frame(WS_BINARY_FRAME, payload, sizeof(payload), frame);
...
static void test_missing_mask(void)
{
    ...
    enum wsFrameType t = wsParseInputFrame(frame, sizeof(frame), &dataPtr, &dataLen);
