...
static void test_fragmented_text_two_frames(void) {
  ...
  size_t f1 = make_frame(frame1, 0, WS_TEXT_FRAME, (const uint8_t *)"Hel", 3);

  size_t f2 = make_frame(frame2, 1, 0x00, (const uint8_t *)"lo", 2);
...
static void test_continuation_without_start(void) {
  uint8_t frame[64];
  size_t f = make_frame(frame, 1, 0x00, (const uint8_t *)"x", 1);
...
static void test_new_text_during_fragment(void) {
  ...
  size_t f1 = make_frame(f1buf, 0, WS_TEXT_FRAME, (const uint8_t *)"A", 1);

  size_t f2 = make_frame(f2buf, 1, WS_TEXT_FRAME, (const uint8_t *)"B", 1);
